...
export function ContextDocumentsPanel({
  ...
  return (
    <div className="flex w-[320px] flex-col border-l border-[#E0E0E0] bg-[#FAFAFA]">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#E0E0E0]">
        <div>
          <h3 className="text-[14px] font-bold text-[#1D1C1D]">Context Docs</h3>
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Add form */}
        {showAddForm && (
          <div className="border-b border-[#E0E0E0] p-4 space-y-3 bg-white">
            <input
              type="text"
              value={newTitle}
              onChange={(e) => setNewTitle(e.target.value)}
              placeholder="Document title"
              className="w-full px-3 py-1.5 text-[13px] text-[#1D1C1D] border border-[#E0E0E0] rounded-md focus:border-[#1264A3] outline-none placeholder-[#ABABAD]"
            />
            <select
              value={newDocType}
              onChange={(e) => setNewDocType(e.target.value as AgentContextDocument["doc_type"])}
              className="w-full px-3 py-1.5 text-[13px] text-[#1D1C1D] border border-[#E0E0E0] rounded-md focus:border-[#1264A3] outline-none bg-white"
            >
            ...
            <textarea
              ...
              rows={6}
              className="w-full px-3 py-2 text-[12px] leading-[1.5] text-[#1D1C1D] border border-[#E0E0E0] rounded-md focus:border-[#1264A3] outline-none resize-y font-mono placeholder-[#ABABAD]"
            />
            <div className="flex gap-2">
              <button
                onClick={handleAdd}
                disabled={!newTitle.trim() || !newContent.trim() || adding}
                className="flex-1 px-3 py-1.5 text-[13px] font-medium rounded-md bg-[#1D1C1D] text-white hover:bg-[#333] disabled:bg-[#E0E0E0] disabled:text-[#ABABAD] transition-colors"
              >
                {adding ? "Adding..." : "Add Document"}
              </button>
              <button
                onClick={() => setShowAddForm(false)}
                className="px-3 py-1.5 text-[13px] text-[#616061] rounded-md hover:bg-[#E0E0E0]/50 transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="py-1">
          {documents.map((doc) => {
            ...
            return (
              <div key={doc.id} className="border-b border-[#F0F0F0]">
                <button
                  onClick={() => setExpandedDoc(isExpanded ? null : doc.id)}
                  className="flex w-full items-center gap-2 px-4 py-2.5 text-left hover:bg-white transition-colors"
                >
                  {isExpanded ? (
                    <ChevronDown className="h-3.5 w-3.5 text-[#ABABAD] shrink-0" />
                  ) : (
                    <ChevronRight className="h-3.5 w-3.5 text-[#ABABAD] shrink-0" />
                  )}
                  <FileText
                    className="h-3.5 w-3.5 shrink-0"
                    style={{ color: DOC_TYPE_COLORS[doc.doc_type] }}
                  />
                  <span className={`text-[13px] truncate flex-1 ${doc.is_active ? "text-[#1D1C1D]" : "text-[#ABABAD] line-through"}`}>
                    {doc.title}
                  </span>
                  <span
                ...
                {isExpanded && (
                  <div className="px-4 pb-3 ml-5">
                    <pre className="text-[12px] leading-[1.5] text-[#616061] font-mono whitespace-pre-wrap bg-[#F8F8F8] rounded-md p-3 max-h-[200px] overflow-y-auto border border-[#E0E0E0]">
                      {doc.content}
                    </pre>
